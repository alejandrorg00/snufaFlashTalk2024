...
DA=0;
DA0 = 0.002;          % level of dopamine above the baseline
rew = [];
...
for sec = 1:T                          % simulation of T seconds
    for t = 1:1000                     % simulation of 1 sec
        ...
        DA = DA * 0.995;
        if (mod(t,10) == 0)
            s(1:Ne,:) = max(0, min(sm, s(1:Ne,:) + (DA0 + DA) * sd(1:Ne,:)));
            sd = 0.99 * sd;
        end
        if any(fired == n1)
            n1f = [n1f, sec * 1000 + t];
...
text(-50+tin*1000, y_n1, 'pre', 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle');
text(-50+tin*1000, y_n2, 'post', 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle');

hold off;

% Adjust axis limits to reduce the distance between neurons
axis([tin* 1000 T* 1000, 0.5, 2.5]); % Y-axis from 0.5 to 2.5 to compress spacing
...
text(x0 + 10, y0 + 0.5, '1 s', 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top'); % Label the scale bar
...
text(x_text+tin, y_text, 'Synaptic strength', 'Color', color_synaptic, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');
...
x_text = 0.001 * 1; % Beginning of the x-axis
y_text = ylimits(2); % Top of the y-axis
text(x_text+tin, y_text, 'Eligibility trace', 'Color', color_eligibility, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');
...
x_text = 0.001 * 1; % Beginning of the x-axis
y_text = ylimits(2); % Top of the y-axis
text(x_text+tin, y_text, 'Dopamine', 'Color', color_dopamine, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');
